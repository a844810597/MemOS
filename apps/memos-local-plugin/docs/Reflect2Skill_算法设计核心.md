...
#### Round 2 · 任务 B："部署 Web 应用到 Docker"（完全不同的任务目标）
...
新 L1 trace $f^{(1)}_{2,1}$ 写入 → 检索候选池 → 发现 $f^{(1)}_{1,1}$ 的 error pattern（容器 + pip 编译失败 + 系统库缺失）**高度相似**。两个 trace 来自完全不同的任务（任务 A 是安装 lxml，任务 B 是部署 Web 应用），但遇到了**相同类型的子问题**。
